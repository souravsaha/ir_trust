...
def get_keras_train_input(pair_file, histogram_file):
    ...
    # AS OF CHANGING the #QueryTerm = 60 to 18 
    # np histogram
    histogram_input = np.zeros((data_count, query_term_maxlen, 10), dtype=np.float32)

    # topic idf
    idf_input = np.zeros((data_count, query_term_maxlen, 1), dtype=np.float32)
...
def load_histogram_data(filepath):
    ...
    with open(filepath, 'r') as inputFile:
        for line in inputFile:
            ...
            # handle idfs
            #
            idfs = np.zeros((query_term_maxlen, 1), np.float32)
            #print(parts)
