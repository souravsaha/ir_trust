...
# print(len(qrels_doc_pairs[topic][1]))
...
with open(prerank_file, 'r') as inputFile:
    for line in inputFile:
        ...
        prerank_doc_pairs[parts[0]].append(parts[1].strip())
        count_prerank+=1
...
def create_1_0_pairs(topics):
    ...
    for topic in topics:
        # print("Doc ", len(qrels_doc_pairs[topic][1]))
        for positive in qrels_doc_pairs[topic][0]:
            if len(qrels_doc_pairs[topic][1]) > 0 :
...
def writeOutFiles(train1,train2,train3,train4,test,foldnumber):
    ...
    with open('../data/5-folds/msmarco/'+str(qrel_name)+'_fold_'+str(foldnumber)+'.train', 'w') as trainFile:
    ...
    with open('../data/5-folds/msmarco/' + str(qrel_name) + '_fold_' + str(foldnumber) + '.test', 'w') as testFile:
        lines = []
        for topic in test:
...
if not os.path.exists('../data/5-folds/msmarco/'):
    os.makedirs('../data/5-folds/msmarco/')
