...
if arg_qrel_or_preranked == 'prerank':
    with open(arg_preranked_or_qrel_file, 'r') as inputFile:
        for line in inputFile:
            parts = line.split("\t")
            topic_doc_pairs.append((parts[0].strip(), parts[1].strip(), parts[2].strip()))

if arg_qrel_or_preranked == 'qrel':
    with open(arg_preranked_or_qrel_file, 'r') as inputFile:
        for line in inputFile:
            parts = line.split()
            topic_doc_pairs.append((parts[0].strip(), parts[2].strip(), parts[3].strip()))
...
model = gensim.models.KeyedVectors.load_word2vec_format(arg_embedding_file, binary=True)
vectors = model.wv
del model
...
with open(arg_corpus_file, 'r') as inputFile:
    for line in inputFile:
        ...
        trec_text_collection_data.append(parts[1].strip())

        for w in parts[1].split(' '):
            ws = w.strip()
            if ws in vectors.vocab:
                trec_corpus[parts[0].strip()].append(vectors.vocab[ws].index)
...
with open(arg_topics_file, 'r') as inputFile:
    for line in inputFile:
        ...
        if parts[0].strip() not in trec_topics:
            trec_topics[parts[0].strip()] = []

        for w in parts[1].split(' '):
            ws = w.strip()
            if ws in vectors.vocab:
                trec_topics[parts[0].strip()].append(vectors.vocab[ws].index)

        if len(trec_topics[parts[0].strip()]) > max_topic_word_count:
            max_topic_word_count = len(trec_topics[parts[0].strip()])
...
with open('../data/drmm_bm25'+'_msmarco_test_histogram_'+str(arg_bin_size)+'.txt', 'w') as outputFile:
